...
vector< vector<TreeNode*> > Nlp::get_possible_chunkings(vector<TreeNode*> nodes)
{
    ...
    {
        auto sub_nodes = get_sub_nodes(nodes, i, nodes.size());
        auto chunked_node = greedy_match(sub_nodes);
        if (chunked_node != NULL)
        {
            vector<TreeNode*> current;
            for (int j = 0; j < i; j++)
            {
                current.push_back(nodes[j]);
            }
            current.push_back(chunked_node);
            for (int k = i + chunked_node->children.size(); k < nodes.size(); k++)
            {
                current.push_back(nodes[k]);
            }
...
TreeNode* Nlp::parse(string sentence)
{
    ...
    {
        auto pos = get_token_pos(tokens[i]);
        auto leaf = new TreeNode(pos, tokens[i], {});
...
TreeNode* Nlp::parse_rec(vector<TreeNode*> nodes)
{
    ...
    {
        auto result = parse_rec(chunkings[i]);
