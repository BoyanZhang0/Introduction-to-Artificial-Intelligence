...
int heuristic::misplace(std::vector<int>& state, std::vector<int>& goalState)
{
	...
	*/
	...
	{
		if (state[i] && state[i] != goalState[i]) h1++;
	}
	return h1;
}
...
int heuristic::manhattan(std::vector<int>& state, std::vector<int>& goalState)
{
	...
	*/
	...
	for (int i = 0; i < state.size(); i++)
	{
		if (state[i])
		{
			Position tempGoal = getPosition(state.size(), state[i]);
			Position tempCurrent = getPosition(state.size(), i);
			h2 += abs(tempCurrent.col - tempGoal.col) + abs(tempCurrent.row - tempGoal.row);
		}
	}
	return h2;
}
